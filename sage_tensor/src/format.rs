...
{
    format_inner(tensor, f, format, fmt_opt, 0, tensor.order())
}
    ...
    F: FnMut(&T, &mut fmt::Formatter<'_>) -> fmt::Result + Clone,
{
    match tensor.shape() {
        &[len] => {
            let buffer_data = tensor.buffer.as_native().data();
            f.write_str("[")?;
            format_with_overflow(
                ...
                ", ",
                ELLIPSIS,
                &mut |f, index| format(&buffer_data[tensor.mem_layout.translate_default(index)], f),
            )?;
            f.write_str("]")?;
        }
        shape => {
    ...
    T: Element + fmt::Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        format(&self.assign(Backend::Native), f, <_>::fmt, &fmt_opt)
    }
}
